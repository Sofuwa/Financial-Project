...
def Company(single_ticker, ticker_selected, tabs):
    with tabs [0]:
        ...
        company_officers = Ticker(single_ticker).company_officers
        company_officers = company_officers.reset_index(drop=False)
        company_officers = company_officers.iloc[:,1:-1]
        company_officers = company_officers.drop(['fiscalYear'], axis=1)
