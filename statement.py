...
def Statements(tabs,single_ticker,ticker_selected):
    with tabs[5]:
        ...
        @st.cache(allow_output_mutation=True,suppress_st_warning=True, show_spinner=False)    
        def get_income_stmt(ticker_select,yearly=True):
            income = yf_si.get_income_statement(single_ticker, yearly = False)
            income = income.reset_index()
            income['Breakdown'] = income['Breakdown'].str.findall('.[^A-Z]*').apply(' '.join)
            income['Breakdown'] = income['Breakdown'].str.title()
            income.columns = [i[0].replace('-','/') for i in income.columns.astype("str").str.split()]
            income = income.fillna(0)
            income.iloc[:,1:] = income.iloc[:,1:].astype(int)
            income.iloc[:,1:] = income.iloc[:,1:].applymap('{:,}'.format)
            income.iloc[:,1:] = income.iloc[:,1:].astype(str).replace('0','N/A')
            income = income.set_index('Breakdown')
            return income
        
        @st.cache(allow_output_mutation=True,suppress_st_warning=True, show_spinner=False)      
        def get_stmt(ticker_select, select_fin, select_fin_time, single_tick = 'None'):
            stmt_data = get_income_stmt(ticker_select,yearly=True)
            ...
            if select_fin == 'Balance Sheet':
                ...
                if select_fin_time == 'Annual':
                    ...
                    stmt_data = get_income_stmt(single_tick,yearly=True)
                else:
                    st.header(single_tick + "'s " + select_fin_time +' Income Statement')
                    stmt_data = get_income_stmt(single_tick,yearly=False)
